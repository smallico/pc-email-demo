<template>
  <div id="sendbox">
    ...
    <div class="el-table-box">
      <p class="send">共{{trashTotal}}封邮件</p>
      <div class="table-box">
        <template v-for="(item,index) in trashBoxTableData">
          <p
            class="table-title"
            ...
            >
              <template slot-scope="scope">
                <div class="operation-div">
                  ...
                  <el-button type="text" @click="deleteRow(scope.row,index,scope.$index)">清除</el-button>
    ...
    <div class="pagination-box">
      <p>共{{trashTotal}}条记录 当前第1页</p>
      <el-pagination layout="prev, pager, next" :total="Number(trashTotal)"></el-pagination>
    </div>
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("trashBoxTableData")) {
      this.updateTrashBoxTableData(
        JSON.parse(sessionStorage.getItem("trashBoxTableData"))
      );
      this.computedTotal();
    } else {
      this.getTableData();
  ...
  methods: {
    ...mapMutations(["updateTrashBoxTableData", "updateTrashTotal"]),
    //获取并组装表格数据
    getTableData() {
      request("/trashBox", data => {
        console.log("", data);
        var tableData = [];
        tableData.push({
          title: "",
        ...
        this.updateTrashBoxTableData(tableData);
        this.updateTrashTotal(data.data.total);
      });
    },
    //查看邮件详情
    ...
    search() {
      ...
      if (this.keyWord.trim() !== "") {
        this.trashBoxTableData.forEach((item, tIndex) => {
          if (tIndex > 0) {
            this.trashBoxTableData[tIndex].tdata = this.trashBoxTableData[
              tIndex
            ].tdata.filter(item => {
              if (
    ...
    computedTotal() {
      ...
      this.trashBoxTableData.forEach((item, tIndex) => {
        if (tIndex > 0) {
          totalNum += this.trashBoxTableData[tIndex].tdata.length;
        }
      });
      this.updateTrashTotal(totalNum);
    },
    //高级搜索
    advancedSearch() {
    ...
    manual_go(row) {
      if (this.signLabel == "签收意见") {
        this.trashBoxTableData[this.tIndex].tdata[
          this.manual_index
        ].isRecepted = 1;
        this.trashBoxTableData[this.tIndex].tdata[
          this.manual_index
      ...
      this.updateTrashBoxTableData(this.trashBoxTableData);
      this.manualDialogVisible = false;
    },
    //删除某一行
    deleteRow(row, tIndex, index) {
      this.$confirm("确定要清除吗?", "提示", {
        ...
        .then(() => {
          this.trashBoxTableData[tIndex].tdata.splice(index, 1);
          this.updateTrashBoxTableData(this.trashBoxTableData);
          this.$message({
            type: "success",
            message: "清除成功!"
          });
          this.updateTrashTotal(this.trashTotal - 1);
        })
        .catch(() => {
          this.$message({
            type: "info",
    ...
    selectAll(selection) {
      //同时选中多个table多选框
      //selection代表已选中的数量
      this.trashBoxTableData.forEach((item, index) => {
        if (index > 0) {
  ...
  computed: {
    ...
    unread_num() {
      ...
      this.trashBoxTableData.forEach((item, tIndex) => {
        if (tIndex > 0) {
          this.trashBoxTableData[tIndex].tdata.forEach(item => {
            if (item.isOpened == 0) {
              num++;
            }
          });
    ...
    ...mapState(["trashBoxTableData", "trashTotal"])
  }
};
</script>
