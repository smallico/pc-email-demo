<template>
  <div id="sendbox">
    ...
    <div class="search">
      ...
      <el-dialog :visible.sync="dialogVisible" width="50%">
        <el-form :model="form" :inline="true" size="small" ref="advancedForm">
          ...
          <el-form-item label="发件人/终端" :label-width="formLabelWidth" prop="sender">
            <el-input v-model="form.sender" auto-complete="off" class="el-ipt"></el-input>
          </el-form-item>
    ...
    <div class="el-table-box">
      <p class="send">
        共{{receiveTotal}}封邮件（
        <span>未读邮件{{unread_num}}封，未签收邮件{{unsign_num}}封）</span>
      </p>
      <div class="table-box">
        <template v-for="(item,index) in receiveBoxTableData">
          <p
            class="table-title"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <img :src="scope.row.isOpened==1?read_url:no_read_url" alt @click="f(scope.row.id)">
              </template>
            </el-table-column>
            <el-table-column
              prop="receiveTime"
    ...
    <div class="pagination-box">
      <p>共{{receiveTotal}}条记录 当前第1页</p>
      <el-pagination layout="prev, pager, next" :total="Number(receiveTotal)"></el-pagination>
    </div>
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("receiveBoxTableData")) {
      this.updateReceiveBoxTableData(
        JSON.parse(sessionStorage.getItem("receiveBoxTableData"))
      );
      this.computedTotal();
    } else {
      this.getTableData();
  ...
  methods: {
    ...mapMutations(["updateReceiveBoxTableData", "updateReceiveTotal"]),
    //获取并组装表格数据
    getTableData() {
      request("/receiveBox", data => {
        console.log("", data);
        var tableData = [];
        tableData.push({
          title: "",
        ...
        this.updateReceiveBoxTableData(tableData);
        this.updateReceiveTotal(data.data.total);
      });
    },
    //查看发件详情
    checkDetail(row, tIndex, index) {
      this.receiveBoxTableData[tIndex].tdata[index].isOpened = 1;
      this.updateReceiveBoxTableData(this.receiveBoxTableData);
    ...
    search() {
      ...
      if (this.keyWord.trim() !== "") {
        this.receiveBoxTableData.forEach((item, tIndex) => {
          if (tIndex > 0) {
            this.receiveBoxTableData[tIndex].tdata = this.receiveBoxTableData[
              tIndex
            ].tdata.filter(item => {
              if (
    ...
    computedTotal() {
      ...
      this.receiveBoxTableData.forEach((item, tIndex) => {
        if (tIndex > 0) {
          totalNum += this.receiveBoxTableData[tIndex].tdata.length;
        }
      });
      this.updateReceiveTotal(totalNum);
    },
    //高级搜索
    advancedSearch() {
    ...
    manual_go(row) {
      if (this.signLabel == "签收意见") {
        this.receiveBoxTableData[this.tIndex].tdata[
          this.manual_index
        ].isRecepted = 1;
        this.receiveBoxTableData[this.tIndex].tdata[
          this.manual_index
    ...
    deleteRow(row, tIndex, index) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        .then(() => {
          this.receiveBoxTableData[tIndex].tdata.splice(index, 1);
          this.updateReceiveBoxTableData(this.receiveBoxTableData);
          this.$message({
            type: "success",
            message: "删除成功!"
          });
          this.updateReceiveTotal(this.receiveTotal - 1);
        })
        .catch(() => {
          this.$message({
            type: "info",
    ...
    selectAll(selection) {
      //同时选中多个table多选框
      //selection代表已选中的数量
      this.receiveBoxTableData.forEach((item, index) => {
        if (index > 0) {
  ...
  computed: {
    ...
    unread_num() {
      ...
      this.receiveBoxTableData.forEach((item, tIndex) => {
        if (tIndex > 0) {
          this.receiveBoxTableData[tIndex].tdata.forEach(item => {
            if (item.isOpened == 0) {
              num++;
            }
          });
    ...
    ...mapState(["receiveBoxTableData", "receiveTotal"])
  }
};
</script>
