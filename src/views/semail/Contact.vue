<template>
    <div>
        <div class="content" v-show="emailType===0">
            <div class="left-wrap">
                <div class="left">
            ...
            <div class="right" v-if="status == 0">
                <div class="rightHead search-top" style="position:relative;">
                    <div>
                        <el-button type="primary" size="small" @click="open('cform',0)">申请新建联系人
                        </el-button>
                      ...
                      >移动</el-button>
                        ...
                        <el-button style="margin-left:-1px;" size="mini" @click="del" v-if="selectStatus==1">删除
                        </el-button>
                    </div>
                    <div>
                        <el-input
                    ...
                    >
                        ...
                        <el-table-column width="70">
                            <template slot-scope="scope">
                                ...
                                <span
                                        class="el-icon-star-off star cur-p"
                        ...
                        <el-table-column width="250" label="操作">
                            <template slot-scope="scope">
                                ...
                                <el-button size="small" type="text" @click="edit(scope.row,scope.$index)">编辑</el-button>-->
            ...
            <el-dialog title="新建联系人" :visible.sync="dialogFormVisible1">
                <el-form :model="cform" :rules="rules" inline size="small" ref="cform">
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button @click="close('cform')" size="small">取 消</el-button>
                    <el-button @click="csure('cform')" type="primary" size="small">确定</el-button>
                </div>
            </el-dialog>
            <el-dialog title="编辑信息" :visible.sync="dialogFormVisible3">
                <el-form :model="cform">
                    <el-form-item label="姓名">
                        <el-input v-model="eform.name" auto-complete="off" placeholder="请输入姓名"></el-input>
                    </el-form-item>
                    <el-form-item label="邮箱">
                        <el-input v-model="eform.email" auto-complete="off" placeholder="请输入邮箱"></el-input>
                    </el-form-item>
                    <el-form-item label="电话">
                        <el-input v-model="eform.phone" auto-complete="off" placeholder="请输入手机号码"></el-input>
                    </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
        ...
        <div class="content" v-show="emailType===1">
            ...
            <div class="right" v-show="status == 0">
                <div class="rightHead search-top" style="position:relative;">
                    <div>
                        <el-button type="primary" size="small" @click="open('cform',1)">申请新建终端</el-button>
                ...
                <div class="rightContent">
                    ...
                    >
                        ...
                        <el-table-column prop="email" label="终端邮件地址"></el-table-column>
                        <!--                        <el-table-column prop="phone" label="" v-if="emailType===1"></el-table-column>-->
                        <el-table-column label="终端所在分组">
                            <template slot-scope="scope">
                                <span v-if="scope.row.groupId==1">常用终端</span>
                                <span v-if="scope.row.groupId==2">星标终端</span>
                                <span v-if="scope.row.groupId==3">自定义分组一</span>
                                <span v-if="scope.row.groupId==4">自定义分组二</span>
                            </template>
                        </el-table-column>
            ...
            <el-dialog title="新建终端邮箱" :visible.sync="otherDialogFormVisible">
                <el-form :model="cform" :rules="rules" inline size="small" ref="cform">
                    <el-form-item label="终端名称" prop="name">
                        <el-input v-model="cform.name" auto-complete="off"
                                  placeholder="请输入终端名称"></el-input>
                    </el-form-item>
                    <el-form-item label="终端邮箱" prop="email">
                        <el-input v-model="cform.email" auto-complete="off"
                                  placeholder="请输入终端邮箱"></el-input>
                    </el-form-item>
                    <el-form-item label="终端地址" prop="url">
                        <el-input v-model="cform.url" auto-complete="off"
                                  placeholder="请输入终端地址"></el-input>
                    </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
                    <el-button @click="close('cform')" size="small">取 消</el-button>
    ...
    export default {
        ...
        created() {
            ...
            request("/groupClientData", data => {
                console.log("group", data);
                this.otherData2 = data.data.group;
                this.allData = data.data.allData;
                this.otherList = this.allData[0].children;
                this.data = this.allData[0].children;
                this.total = this.list.length;
            });
            request("/conOrganClientData", data => {
                this.otherData1 = data.data;
            });
        },
        methods: {
            // 手机校验
            ...
            write(row) {
                console.log("我要去写信");
                this.$router.push({name: "send", params: {email: row.email, name: row.name}});
            },
            nearby(a) {
                console.log("我选择了");
                //this.groupsIndex = 0;
            ...
            csure(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.notRepeat(this.cform.name)) {
                            this.list.push(this.clone(this.cform));

                            this.dialogFormVisible1 = false;
                            this.otherDialogFormVisible = false;
                            for (let key in this.cform) {
                                this.cform[key] = ''
                            }
                        } else {
                            this.$message({
...
//             }
                // if (this.isActive == row.userId) {                    //取消星级选择
                //   this.isActive = 0;
                //   this.data2=this.log;
                // } else {                                            //点击进行星级选择
                //   this.log=JSON.parse(JSON.stringify(this.data2));
                //   this.isActive = userId;
                //      for (var i = 0; i < this.data2.length; i++) {
                //     var starGroupChildrenList = this.data2[i].children;

                //     for (var j = 0; j < starGroupChildrenList.length; j++) {     //将选中的联系人从原本分组中移除
                ...
                //   //splice
                //   //concat
                //   this.data2.forEach((item, index) => {                       //将选中的添加到星标分组中
                //     if (item.groupId == 2) {
                //       var oldList = this.data2[index].children;
                //       var arr = [];
                //       arr.push(row);
